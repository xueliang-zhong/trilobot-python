...
class AutonomousCarConfig:
    ...
    breadcrumb_cells: int = 12                      # angular sectors for breadcrumb map
    breadcrumb_visit_threshold: float = 3.0         # seconds of heading to count as "visited"
    breadcrumb_decay: float = 0.999                 # per-loop decay for breadcrumb memory
    breadcrumb_novelty_bonus: float = 8.0           # score bonus for heading toward unvisited sectors
    breadcrumb_visit_penalty: float = 3.0           # mild penalty for revisiting same sector

    open_space_beacon_cells: int = 8                # sectors for open-space beacon memory
    open_space_beacon_threshold: float = 80.0       # front distance to register as open-space beacon
    open_space_beacon_gain: float = 10.0            # score bonus for heading toward beacon
    open_space_beacon_decay: float = 0.998          # per-loop decay for beacon memory
    open_space_beacon_max_age: float = 30.0         # seconds before beacon expires

    multi_step_lookahead_steps: int = 4             # steps for improved lookahead simulation
    multi_step_lookahead_corridor_weight: float = 0.4  # weight of corridor support in lookahead
    multi_step_lookahead_angle_cost: float = 0.15   # cost per degree of heading change in lookahead

    obstacle_naming_threshold: float = 0.7          # persistence score to name an obstacle
    obstacle_names: Tuple[str, ...] = (
        "Sir Blocks-a-Lot", "The Wall", "Captain Obstacle", "Bumper",
        "Inconvenience", "The Door That Isn't", "Mystery Object",
        "The Uninvited Guest", "Plot Twist", "The Usual Suspect",
        "Barrier McBlockface", "The Impediment", "Obstruction",
        "The Wall of Regret", "Blocky McBlockface", "The Nuisance",
        "The Imposter Wall", "The Phantom Obstacle", "The Decoy",
        "The Troll", "The Ambush", "The Surprise",
    )
...
class AutonomousCarController:
    ...
    breadcrumb_map: Dict[int, float] = field(default_factory=dict)
    breadcrumb_visit_times: Dict[int, float] = field(default_factory=dict)
    open_space_beacon: Dict[int, float] = field(default_factory=dict)
    last_open_space_heading: int = field(default=0)
    last_open_space_time: float = field(default=-math.inf)
    named_obstacles: Dict[int, str] = field(default_factory=dict)
    next_obstacle_name_idx: int = field(default=0)
    obstacle_conversations: Dict[str, int] = field(default_factory=dict)
    ...
    def __post_init__(self):
        ...
        for i in range(self.config.breadcrumb_cells):
            self.breadcrumb_map[i] = 0.0
            self.breadcrumb_visit_times[i] = 0.0
        for i in range(self.config.open_space_beacon_cells):
            self.open_space_beacon[i] = 0.0
    ...
    def score_heading(
        ...
        score += self.breadcrumb_novelty_score(angle) * self.get_scoring_weight("breadcrumb")

        if now is not None:
            score += self.open_space_beacon_score(angle, now) * self.get_scoring_weight("open_space_beacon")
    ...
    def get_quip(self, mode: str, front_distance: float, heading: int, now: float) -> str:
        ...
        if now - self.last_self_aware_quip_time > self.config.self_aware_quip_cooldown and self.total_loops > 100:
            ...
            if self_aware_candidates:
                ...
                return quip
        if self.named_obstacles and mode == "drive" and self.total_loops % 100 == 0:
            for angle, name in list(self.named_obstacles.items())[:3]:
                dist = self.sanitize_distance(self.last_scan.get(angle, 0.0))
                if dist > 0:
                    candidates.append(f"still seeing {name} at {angle}° ({dist:.0f}cm). {personality} remembers")
                    candidates.append(f"{name} hasn't moved. {personality} is starting to think it's permanent")
                    candidates.append(f"encounter #{self.obstacle_conversations.get(name, 0)} with {name}. {personality} and the obstacle have a history")
        if len(self.named_obstacles) >= 3 and mode == "drive" and self.total_loops % 150 == 0:
            names = list(self.named_obstacles.values())[:4]
            candidates.append(f"I've named {len(self.named_obstacles)} obstacles now: {', '.join(names)}. {personality} is building a social circle")
            candidates.append(f"my obstacle friends: {', '.join(names)}. {personality} and the gang")
        unexplored = sum(1 for v in self.breadcrumb_map.values() if v < 0.05)
        total_cells = len(self.breadcrumb_map)
        if unexplored > total_cells * 0.5 and mode == "drive" and self.total_loops % 80 == 0:
            pct = int(unexplored / total_cells * 100)
            candidates.append(f"{pct}% of directions still unexplored. {personality} has work to do")
            candidates.append(f"so much room to discover ({pct}% untouched). {personality} is excited")
        if self.last_open_space_heading != 0 and now - self.last_open_space_time < 20.0 and mode == "drive":
            beacon_dir = "L" if self.last_open_space_heading < 0 else "R"
            candidates.append(f"freedom is {beacon_dir} — I remember open space that way. {personality} has a plan")
            candidates.append(f"breadcrumb trail points {beacon_dir}. {personality} knows the way out")
    ...
    def breadcrumb_cell_for_angle(self, angle: int) -> int:
        """Map an angle to a breadcrumb cell."""
        max_angle = max(abs(a) for a in self.config.scan_angles) or 1
        normalized = (angle + max_angle) / (2.0 * max_angle)
        cell = int(normalized * self.config.breadcrumb_cells)
        return max(0, min(self.config.breadcrumb_cells - 1, cell))

    def update_breadcrumbs(self, heading: int, dt: float):
        """Track which directions have been explored and for how long."""
        decay = self.config.breadcrumb_decay
        for cell in self.breadcrumb_map:
            self.breadcrumb_map[cell] *= decay
        cell = self.breadcrumb_cell_for_angle(heading)
        self.breadcrumb_map[cell] = min(1.0, self.breadcrumb_map[cell] + dt * 0.1)
        self.breadcrumb_visit_times[cell] = self.breadcrumb_visit_times.get(cell, 0.0) + dt

    def breadcrumb_novelty_score(self, angle: int) -> float:
        """Bonus for heading toward unexplored directions, penalty for revisiting."""
        cell = self.breadcrumb_cell_for_angle(angle)
        visit = self.breadcrumb_map.get(cell, 0.0)
        if visit < 0.05:
            return self.config.breadcrumb_novelty_bonus
        if visit < 0.3:
            return self.config.breadcrumb_novelty_bonus * (1.0 - visit / 0.3)
        return -self.config.breadcrumb_visit_penalty * visit

    def beacon_cell_for_angle(self, angle: int) -> int:
        """Map an angle to an open-space beacon cell."""
        max_angle = max(abs(a) for a in self.config.scan_angles) or 1
        normalized = (angle + max_angle) / (2.0 * max_angle)
        cell = int(normalized * self.config.open_space_beacon_cells)
        return max(0, min(self.config.open_space_beacon_cells - 1, cell))

    def update_open_space_beacon(self, front_distance: float, heading: int, now: float):
        """Remember directions that led to open space — a breadcrumb trail to freedom."""
        decay = self.config.open_space_beacon_decay
        for cell in list(self.open_space_beacon.keys()):
            self.open_space_beacon[cell] *= decay
            if self.open_space_beacon[cell] < 0.02:
                del self.open_space_beacon[cell]
        if front_distance > self.config.open_space_beacon_threshold:
            cell = self.beacon_cell_for_angle(heading)
            self.open_space_beacon[cell] = min(1.0, self.open_space_beacon.get(cell, 0.0) + 0.15)
            self.last_open_space_heading = heading
            self.last_open_space_time = now

    def open_space_beacon_score(self, angle: int, now: float) -> float:
        """Bonus for heading toward remembered open-space directions."""
        if now - self.last_open_space_time > self.config.open_space_beacon_max_age:
            return 0.0
        cell = self.beacon_cell_for_angle(angle)
        strength = self.open_space_beacon.get(cell, 0.0)
        if strength > 0.05:
            return strength * self.config.open_space_beacon_gain
        return 0.0

    def name_obstacle(self, angle: int) -> str:
        """Give a persistent obstacle a name for personality and humor."""
        if angle in self.named_obstacles:
            return self.named_obstacles[angle]
        names = self.config.obstacle_names
        idx = self.next_obstacle_name_idx % len(names)
        name = names[idx]
        self.named_obstacles[angle] = name
        self.next_obstacle_name_idx += 1
        return name

    def update_obstacle_naming(self, scan: Dict[int, float]):
        """Check for persistent obstacles that deserve naming."""
        for angle, persistence in self.persistence_map.items():
            if persistence > self.config.obstacle_naming_threshold and angle not in self.named_obstacles:
                self.name_obstacle(angle)
                dist = self.sanitize_distance(scan.get(angle, 0.0))
                self.obstacle_conversations[self.named_obstacles[angle]] = int(dist)

    def multi_step_lookahead(self, start_angle: int, scan: Dict[int, float], steps: int) -> float:
        """Simulate future positions with corridor support awareness."""
        total_score = 0.0
        current_angle = start_angle
        max_angle = max(abs(a) for a in self.config.scan_angles) or 1
        ordered = tuple(sorted((a, self.sanitize_distance(scan.get(a, 0.0))) for a in scan))
        for step in range(steps):
            dist = self.sanitize_distance(scan.get(current_angle, 0.0))
            if dist <= 0.0:
                return total_score - 50.0 * (steps - step)
            projected = dist - step * self.config.lookahead_distance_per_step
            if projected < self.config.danger_distance + 5.0:
                return total_score - 80.0 * (steps - step)
            idx = None
            ...
            if idx is not None:
                support, _ = self.corridor_support(ordered, idx)
            angle_cost = abs(current_angle) * self.config.multi_step_lookahead_angle_cost * step
            step_score = dist * 0.3 + support * self.config.multi_step_lookahead_corridor_weight - angle_cost
            total_score += step_score
            if step + 1 < steps:
                best_next = current_angle
                best_next_score = -math.inf
                for a in scan:
                    if abs(a - current_angle) <= 45:
                        d = self.sanitize_distance(scan.get(a, 0.0))
                        if d > best_next_score:
                            best_next_score = d
                            best_next = a
                current_angle = best_next
        return total_score
    ...
    def plan(self, front_distance: float, now: float) -> MotionCommand:
        ...
        self.update_breadcrumbs(self.current_heading, self.config.loop_delay_s)
        self.update_open_space_beacon(front_distance, self.current_heading, now)
        self.update_obstacle_naming(self.last_scan)
        ...
        if self.last_scan:
            for angle in list(self.last_scan.keys()):
                la_score = self.multi_step_lookahead(angle, self.last_scan, self.config.multi_step_lookahead_steps)
            ...
            if follow_mode:
                ...
                if command.mode == "brave_push":
                    ...
                    if now - last_print_time >= print_interval:
                        ...
                        unexplored = sum(1 for v in controller.breadcrumb_map.values() if v < 0.05)
                        total_bc = len(controller.breadcrumb_map)
                        breadcrumb_tag = f"  [explore:{unexplored}/{total_bc}]" if unexplored > total_bc * 0.3 else ""
                        beacon_tag = f"  [beacon→{controller.last_open_space_heading:+d}]" if controller.last_open_space_heading != 0 and now - controller.last_open_space_time < 20.0 else ""
                        named_tag = f"  [{len(controller.named_obstacles)}named]" if controller.named_obstacles else ""
                        print(f"\r[DRIVE] {arrow}  front={front_distance:5.1f}cm  hdg={command.heading:+4d}  spd={controller.current_speed:.2f}  stress={stress_ch}  [{personality}]{streak}{terrain_tag}{corner_tag}{momentum_tag}{closing_tag}{exit_tag}{recovery_tag}{shape_tag}{dead_end_pred_tag}{junction_tag}{strategy_tag}{patience_tag}{nearmiss_tag}{loop_tag}{wf_tag}{moving_tag}{hysteresis_tag}{side_corridor_tag}{cluster_tag}{flow_tag}{breadcrumb_tag}{beacon_tag}{named_tag}{achievement_tag}{quip_tag}{fortune_tag}  | {emotion}   ", end='', flush=True)
                        on_inline_line = True
                        last_print_time = now
